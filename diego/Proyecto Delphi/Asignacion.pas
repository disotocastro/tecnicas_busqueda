...
var
  ...
  begin
    ...
    begin
      // Realiza una permutación de los índices
      ...
      // Revertir la permutación para continuar con el siguiente ciclo
      Temp := ActualAsignacion[indice];
      ActualAsignacion[indice] := ActualAsignacion[j];
      ActualAsignacion[j] := Temp;
    end;
...
begin
  ...
  // Asignación inicial en orden
  for i := Low(Tabla) to High(Tabla) do
    ActualAsignacion[i] := i;

  ProbarPermutacion(0, 0);

  // Imprimir la mejor asignación y el total de permutaciones
